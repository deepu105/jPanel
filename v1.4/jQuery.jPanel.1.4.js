 ...
 * 
 *	@option ajaxCache	- true or false @default-false
 *						Enable or disable the cache of data loaded via ajax.
 ...
 *	@option defaultOpen	- true or false @default-false
 *						Enable or disable panel default to open in accordion/fieldset view
 ...
 *	@option fieldSet	- true or false @default-false
 *						Enable or disable the fieldset view instead of accordion in panel view. 
...
(function( $ ){
	...
	$.fn.jPanel = function( options ) {
	    ...
	    if ( options ) {
			...
			settings.defaultOpen=(settings.defaultOpen==="true" || settings.defaultOpen===true)? true:false;
			...
			settings.retainView=(settings.retainView==="true" || settings.retainView===true)? true:false;
			settings.fieldSet=(settings.fieldSet==="true" || settings.fieldSet===true)? true:false;
			settings.ajaxCache=(settings.ajaxCache==="true" || settings.ajaxCache===true)? true:false;
	    }
		if( settings.defaultView==="tab" || settings.switchView ){	
			
			// overriding fieldSet view when tab view is enabled.
			settings.fieldSet=false;
			
			// Wrapping all the tab containers together
			...
			if(settings.switchView ){	
				...
				$('div#panelWrapper_'+selector+ '> div > button.jPanelSwitchItem').bind('click',function(e) {
					e.preventDefault();
	...
	function togglePanelView(button,settings,selector){
		...
		if(currentView==='tab'){
			...
			panel.each(function() {
				...
				// retaining the panel state
				setPanel(this,settings);
	...
	function constructPanel(panel,settings,selector) {
		...
		if(settings.defaultView==="tab" || settings.switchView ){
			...
			$('ul#panelUL_'+selector+ '> li > a').unbind("click").bind('click',function(e) {
				e.preventDefault();
				var head=$(this);
				...
				parent.children().children('* > div.ui-widget-content:not('+head.attr('href')+')').hide();
		...
		if(settings.defaultView=="panel" || settings.switchView){
			...
			// Create a new header as the first item within the container. to act as the header (legend in case of fieldSet view)
			if(settings.fieldSet){
				$("<legend id='"+elementId+"_Title' class='head ui-widget-header ui-state-default ui-helper-reset  ui-corner-all'><span class='ui-icon ui-icon-circle-plus leftIcon'></span><a href='#' class='nlink' tabindex='-1'>" + parent.attr("title") + "</a></legend>").prependTo(parent);
				parent.wrapInner("<fieldset class='noPanelBorder'></fieldset>");
			}else{
			...
			$(".head", parent).unbind("click").click(function(e) {
				e.preventDefault();
				var plusIcon = "ui-icon-circle-plus";
				var minusIcon = "ui-icon-circle-minus";
				var head=$(this);
				...
				if(settings.fieldSet){
					head.toggleClass("ui-state-active ui-state-default");
					head.parent().toggleClass('noPanelBorder');
				}else{
	...
	function loadRemoteContent(head,settings){
		...
		var parent=settings.fieldSet?head.parent().parent() : head.parent();

		// building ajax call params from element attributes.
	...
	function setPanel(panel,settings){
		var head;
		if(settings.defaultOpen){
			if(panel.id!==""){
					head=$(panel).find("#"+panel.id+"_Title");
					head.click();
			}
		}else if(settings.cookie){
